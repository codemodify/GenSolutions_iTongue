...
-(void) viewDidLoad {
    ...
    CGFloat x1     = overlayTopLeft.frame.origin.x;
    CGFloat y1     = overlayTopLeft.frame.origin.y;
    CGFloat width  = (overlayTopRight.frame.origin.x   + overlayTopRight.frame.size.width   ) - overlayTopLeft.frame.origin.x;
    CGFloat height = (overlayBottomLeft.frame.origin.y + overlayBottomLeft.frame.size.height) - overlayTopLeft.frame.origin.y;    
    [self moveOverlays:CGRectMake(x1,y1,width,height)];
...
-(IBAction) ocr:(id)sender {
    ...
    @synchronized( self ) {
        ...
        CGImageRef img = [imageResult.image CGImage];
        UIImage* image = [[UIImage imageWithCGImage:img] retain];
