...
-(OcrInputLanguage) getTongueLanguageCodeAsOcrApiInputLanguage:(NSString*)languageCode {
    ...
    if( [languageCode isEqualToString:@"zn-CN"] )
...
-(void) theMethod:(id)image {
    ...
    NSString* text = [self processImage:image];
...
-(void) loadSettings {
    ...
    if( sourceLanguageIndex <= 0 )
        sourceLanguageIndex = 2; // English
    
    if( destinationLanguageIndex <= 0 )
        destinationLanguageIndex = 19; // French
}
...
-(NSString*) processImage:(UIImage*)image {
    ...
    // grayscale
    UIImage* grayScaledImage = [imageSaturate toGrayScale:image];

    NSString* 
        text = [ocrApi analyzeImageAndGetText:[grayScaledImage CGImage]];
        //text = [[NSString alloc] initWithString: [textFromImage stringByReplacingOccurrencesOfString:@"\n" withString:@" "]];
    
//    [self uploadImage:image andText:text];
