...
-(void) loadSettings {
    ...
    // this one is saved in TextMode, so we only read it here
    ...
    if( isThereAnyStrangeSettings ) // Fixes a BUG when opening xMode then Translate then the languages are not set as supposed to 
        [self saveSettings];
}
