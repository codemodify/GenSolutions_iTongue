...
+(OrderedDictionary*) getOutputLanguages {
    
    OrderedDictionary* outputLanguages = [[OrderedDictionary alloc]init];
...
+(OrderedDictionary*) getInputLanguages {
    
    OrderedDictionary* inputLanguages = [[OrderedDictionary alloc]init];
...
-(void) setSourceLanguages {
    ...
    buttonSource.imageView.image = [LanguagePickerViewDataSource getImageByCode:sourceLanguageCode];
    buttonResult.imageView.image = [LanguagePickerViewDataSource getImageByCode:destinationLanguageCode];
...
-(void) saveSettings {
    ...
    NSNumber* isOfflineTranslationModeAsNumber = [NSNumber numberWithInt:isOfflineTranslationMode];
    
    [[NSUserDefaults standardUserDefaults] setObject:sourceLanguageIndexAsNumber forKey:[Helper getSettingsVariableNameForThisVersion:@"sourceLanguageIndex"]];
    [[NSUserDefaults standardUserDefaults] setObject:destinationLanguageIndexAsNumber forKey:[Helper getSettingsVariableNameForThisVersion:@"destinationLanguageIndex"]];
    [[NSUserDefaults standardUserDefaults] setObject:isOfflineTranslationModeAsNumber forKey:[Helper getSettingsVariableNameForThisVersion:@"isOfflineTranslationMode"]];
    [[NSUserDefaults standardUserDefaults] synchronize];    
}

-(void) loadSettings {

    sourceLanguageIndex      = [(NSNumber*) [[NSUserDefaults standardUserDefaults] objectForKey:[Helper getSettingsVariableNameForThisVersion:@"sourceLanguageIndex"]] intValue];
    destinationLanguageIndex = [(NSNumber*) [[NSUserDefaults standardUserDefaults] objectForKey:[Helper getSettingsVariableNameForThisVersion:@"destinationLanguageIndex"]] intValue];
    isOfflineTranslationMode  = [(NSNumber*) [[NSUserDefaults standardUserDefaults] objectForKey:[Helper getSettingsVariableNameForThisVersion:@"isOfflineTranslationMode"]] intValue];
    
    if( sourceLanguageIndex < 0 )
        sourceLanguageIndex = 0;

    bool isThereAnyStrangeSettings = false;
    
    if( sourceLanguageIndex <= 0 ) {
        
        isThereAnyStrangeSettings = true;
        
        sourceLanguageIndex = 1; // English
    }
    ...
    if( destinationLanguageIndex <= 0 ) {
        ...
        destinationLanguageIndex = 18; // French
    }
    
    if( isThereAnyStrangeSettings )
        [self saveSettings];
...
-(void) translateOffline:(id)sender {
    ...
    // get language codes
    NSDictionary* sourceLanguages = [TextViewController getInputLanguages];
    NSDictionary* destinationLanguages = [TextViewController getInputLanguages];
	...
	if( sqlite3_open([sqlLiteDbFilePath UTF8String], &database) == SQLITE_OK ) { // Open the database from the users filessytem
        ...
        NSString* adaptedSentence = [textSource.text stringByReplacingOccurrencesOfString:@"\r\n" withString:@" "];
                  ...
                  adaptedSentence = [adaptedSentence stringByReplacingOccurrencesOfString:@"'" withString:@"&#39;"];
                  adaptedSentence = [adaptedSentence lowercaseString];
...
-(void) viewDidAppear:(BOOL)animated {
    ...
    if( shouldAutoTranslate ) {
        ...
        [NSTimer scheduledTimerWithTimeInterval:0.5 target:self selector:@selector(translate:) userInfo:nil repeats:NO];
...
-(void) alertView:(UIAlertView*)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    // simulate the offline tap
    onlineOffline.selectedSegmentIndex = 1;
    [self setOnlineOflineTranslation:nil];
...
-(void) setOnlineOflineTranslation:(id)sender {
    ...
    isOfflineTranslationMode = (bool)onlineOffline.selectedSegmentIndex;

    // set the same language in the new list
    OrderedDictionary* sourceLanguages = [TextViewController getInputLanguages];
    OrderedDictionary* destinationLanguages = [TextViewController getOutputLanguages];
    ...
    if( isOfflineTranslationMode ) {
        ...
        if( [[sourceLanguages allKeys] containsObject:destinationLanguageCode] ) {
            
            destinationLanguageIndex = [[sourceLanguages allKeys] indexOfObject:destinationLanguageCode];
        }
        else {
        ...
        destinationLanguageIndex = [[destinationLanguages allKeys] indexOfObject:sourceLanguageCode];
    }
    ...
    if( isOfflineTranslationMode ) {
        ...
        UIAlertView* alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString( @"TextMode.ChooseOfflineLanguage", nil )
                                                        message:nil
                                                       delegate:nil
...
-(IBAction) sendMail:(id)sender {
    ...
    NSString* messageBody = [NSString stringWithFormat:@"~~~~~~~~~~ Original Text:<br/>%@<br/><br/>~~~~~~~~~~ Result:<br/>%@<br/><br/>~~~~~~~~~~<br/><a href=\"http://itongue.net\">iTongue.net<a>", textSource.text, textResult.text, nil];
